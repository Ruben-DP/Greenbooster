...
function createDataContext(collectionName: string) {
  function DataProvider({ children }: { children: ReactNode }) {
    ...
    const searchItems = async (searchTerm?: string) => {
      setIsLoading(true);
      try {
        const results = await searchDocuments(collectionName, searchTerm);
        setItems(results);
        setError(null);
      } catch (error) {
...
export const { DataProvider: MeasureProvider, useData: useMeasureData } =
  createDataContext("retrofittingMeasures");
